...
const sixteenthCentury = inventors.filter((inv) => {
  if (inv.year > 1499 && inv.year < 1600){
    return inv;
  }
});
...
const birthYears = inventors.map((inv) => {
  return inv.year;
});
...
const fullNames = inventors.map((inv) => {
...
//   { first: 'Katherine', last: 'Blodgett', year: 1898, passed: 1979 }
// ]
const yearDescending = inventors.sort((a,b) => {
...
const edison = inventors.find((inv) => {
  return inv.last === 'Edison';
});
console.log(edison);